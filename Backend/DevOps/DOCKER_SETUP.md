# Docker Setup Guide for DevOps API
...
3. Start your application with hot-reload on port 5000
...
docker exec devopss-app-dev npm run db:migrate

# Open Drizzle Studio (inside running container)
docker exec devops-app-dev npm run db:studio
```
...
docker exec devops-app-dev npm run db:migrate

# Or connect to Neon Local directly
docker exec devops-neon-local psql -U neon -d neondb
```
...
docker exec devops-app-prod npm run db:migrate

# Or run one-time migration container
docker run --rm -it --env-file .env.production devops-app npm run db:migrate
```
...
docker exec devops-app-prod npm run db:studio
```
...
- **Application**: http://localhost:5000 (configure reverse proxy)
...
# Manual migration
docker exec -it devops-app-dev npm run db:generate
docker exec -it devops-app-dev npm run db:migrate
```
...
- [ ] Visit http://localhost:5000 to verify the application
