<script setup>
  ...
  const isWorkoutComplete = computed(() => {
    ...
    if (!currWorkout) {
      return false; // guard clause to exit function
    }
  ...
  onMounted(() => {
    ...
    if (localStorage.getItem('workouts')) {
      // only enter the if block if we find data saved to the key workouts in localStorage database
      const savedData = JSON.parse(localStorage.getItem('workouts'));
      data.value = savedData;
      selectedDisplay.value = 2; // if they have data saved we do not want them landing on the welcome screen every time they enter the app
